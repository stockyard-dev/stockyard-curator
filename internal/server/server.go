...
import (
	...
	"github.com/stockyard-dev/stockyard-curator/internal/store"
)
...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/curator/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("curator: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("curator: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"recipes": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/curator/")
		return
	}
	var e store.Recipe
...
// update accepts a partial recipe and properly preserves omitted fields.
// Uses json.RawMessage map for proper PATCH semantics — Rating, PrepTime,
// CookTime can all be explicitly set to 0 (e.g. clearing a rating you no
// longer want, or marking a no-cook recipe with cook_time=0).
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if v, ok := raw["ingredients"]; ok {
		json.Unmarshal(v, &patch.Ingredients)
	}
	if v, ok := raw["instructions"]; ok {
		json.Unmarshal(v, &patch.Instructions)
	}
	if v, ok := raw["prep_time"]; ok {
		json.Unmarshal(v, &patch.PrepTime)
	}
	if v, ok := raw["cook_time"]; ok {
		json.Unmarshal(v, &patch.CookTime)
	}
	if v, ok := raw["servings"]; ok {
		json.Unmarshal(v, &patch.Servings)
	}
