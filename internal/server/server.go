package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-curator/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-curator"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/recipes",s.handleListRecipes)
    s.mux.HandleFunc("POST /api/recipes",s.handleCreateRecipe)
    s.mux.HandleFunc("DELETE /api/recipes/{id}",s.handleDeleteRecipe)
    s.mux.HandleFunc("POST /api/meal-plan",s.handlePlan)
    s.mux.HandleFunc("GET /api/meal-plan",s.handleGetPlan)
